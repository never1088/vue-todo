...
<template>
  <main class="app">
    ...
    <ul class="todo_list">
      <!-- v-bind= 디렉티브 데이터 단방향 연결, 리스트는 고유값을 키속성으로 연결하여 순서가 변경돼도 구별가능하도록 한다 -->
      <!-- todo.completed가 true면 completed문자열이 클래스명이됨 -->
      <li
      v-for="(todo, i) in data.items" v-bind:key="todo.id" v-bind:class="{completed:todo.completed}">
        <!-- id가 달라야되므로 id속성과 todo.id값을 단방향연결 -->
        ...
        <button class="remove_btn" type="button" v-on:click="deleteItem(todo.id)">Remove</button>
      </li>
    </ul>
  </main>
</template>
